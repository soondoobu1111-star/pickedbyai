...
```
I asked ChatGPT to recommend my product. It had no idea it existed.
```
...
### Indie Hackers 게시 절차:
1. https://www.indiehackers.com 로그인
2. "New Post" 클릭
3. 위 Title/Body 그대로 복붙
4. `[pickedby.ai](https://pickedby.ai)` → IH 마크다운 링크 형식 확인
5. 태그: `#buildinpublic` `#ai` `#saas`
6. 게시 후 댓글 모니터링 — 모든 댓글에 12시간 이내 답글

### 댓글 답변 전략:
- 긍정 반응 → 감사 + "체크해볼까요?" 제안
- 부정 반응 → "맞는 말입니다. 어떻게 바꾸면 좋을까요?" (방어 금지)
- 제품명 드롭 → 직접 체크 후 스크린샷 공유
- "어떻게 만들었어?" → "Claude Code. 코딩 스킬 제로" 스토리

### 이전 초안 대비 변경점 (5-AI 진단 반영):
1. **"5 AI가 진단한 결과" 섹션 추가** — 정직한 현황 공개 (0 고객, X 정지)
2. **"처방전이 없다" 피드백 반영** — CTA를 "무료 체크해봐" 대신 "너의 의견 들려줘"로 변경
3. **셀러 질문 3개 추가** — 실질적 피드백 수집 목적 (사실상 비동기 인터뷰)
4. **"paying customers" 표현 제거** — 유료 고객 0명 사실 반영
5. **Building in Public 솔직함 강화** — 실패 경험(X 밴, 0 고객) 포함
