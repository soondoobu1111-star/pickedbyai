...
> ⚠️ 주의: 이 측정은 Gemini + Google Search Grounding 기준. 2026-04-06 엔진을 CF Workers AI(llama-3.2-3b)로 교체 후 실시간 웹검색 기능 제거됨. 재측정 시 점수 다를 수 있음.

**AI Visibility Score: 35 / 100** ← 0점에서 즉시 상승 (Gemini 기준)
...
재측정 명령 (현재 엔진: CF Workers AI, 학습 데이터 기반):
```bash
curl -X POST https://api.pickedby.ai/v1/check \
  -H "Content-Type: application/json" \
  -d '{"product":"pickedby.ai"}'
```
