...
function buildTrendSeries(
  ...
  // ── KST 캘린더 날짜 기준 버킷 (daily만) ──────────────────────
  // daily: 마지막 버킷 = 오늘 KST (i=0)
  // weekly/monthly: 기존 슬라이딩 윈도우 유지
  const useKST = tab === 'daily'
  const KST_OFFSET = 9 * 60 * 60 * 1000
  const nowKST = useKST ? new Date(now + KST_OFFSET) : null

  if (useKST) {
    console.log('[trend:kst] rows=' + rows.length + ' firstRow=' + (rows[0]?.created_at || 'none'))
  }

  for (let i = bucketCount - 1; i >= 0; i--) {
    let bucketLabel: string
    let bucketRows: typeof rows

    if (useKST && nowKST) {
      // KST 캘린더: 오늘 KST 날짜에서 i일 전
      const dayKST = new Date(nowKST.getTime())
      dayKST.setUTCDate(dayKST.getUTCDate() - i)
      bucketLabel = dayKST.toISOString().slice(0, 10)
      bucketRows = rows.filter(r => {
        const rKST = new Date(new Date(r.created_at).getTime() + KST_OFFSET)
        return rKST.toISOString().slice(0, 10) === bucketLabel
      })
    } else {
      // 슬라이딩 윈도우 (weekly/monthly)
      const bucketEnd = now - i * bucketSize * 86400000
      const bucketStart = bucketEnd - bucketSize * 86400000
      bucketLabel = toKSTDate(bucketStart)
      bucketRows = rows.filter(r => {
  ...
  if (useKST) {
    const nonEmpty = series.filter(b => b.score !== null).length
    console.log('[trend:kst] buckets=' + series.length + ' nonEmpty=' + nonEmpty + ' last=' + series[series.length - 1]?.bucket_start)
  }
  return series
}
