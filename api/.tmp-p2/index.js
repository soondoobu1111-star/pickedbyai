...
var MAX_DAILY_PROBES = 100;
function buildProbePrompt(template, productName, category) {
  const cat = category || productName;
  return {
    system: template.systemPrompt,
    user: template.userPrompt.replace(/\{product\}/g, productName).replace(/\{category\}/g, cat)
  };
}
__name(buildProbePrompt, "buildProbePrompt");
async function probePerplexityTemplate(apiKey, name, template, category) {
  const safeName = sanitizeForPrompt(name);
  const prompts = buildProbePrompt(template, safeName, category ? sanitizeForPrompt(category) : void 0);
...
__name(probePerplexityTemplate, "probePerplexityTemplate");
...
async function logProbes(env, productName, probes, opts) {
  ...
  const rows = probes.map((p) => ({
    ...
    query_template: p.templateId || opts.queryTemplate || "product_knowledge",
    ai_source: p.ai,
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(`[ProbeLog] insert failed (${status}):`, body);
    } else
      console.log(`[ProbeLog] \u2713 ${rows.length} rows logged for "${productName}" (${status})`);
  } catch (err) {
    console.error("[ProbeLog] fetch error:", err instanceof Error ? err.message : String(err));
  }
}
